...
Run the following command to process HO3D annotations. A new folder processed/ will be created under <project_dir>/ho3d_v3/:

```bash
cd <project_dir> && python scripts/process_ho3d.py
```
Next, download the  [YCB object models ](https://github.com/shreyashampali/ho3d) extract them to `<project_dir>/ho3d_v3/models`.

After completing the steps above, your HO3D data directory should look like this:
```text
MagicHOI/
└── ho3d_v3/
    ├── calibration/          # extracted from HO3D_v3.zip
    │   ├── ABF1/
    │   ├── AP1/
    │   └── …     
    ├── evaluation/           # extracted from HO3D_v3.zip
    │   ├── AP10/
    │   ├── MPM10/
    │   └── …    
    ├── evaluation.txt        # extracted from HO3D_v3.zip
    ├── manual_annotations/   # extracted from HO3D_v3.zip
    ├── models/               # extracted from YCB object models
    │   ├── 002_master_chef_can
    │   ├── 003_cracker_box
    │   └── …      
    ├── processed/            # generated by scripts/process_ho3d.py
    │   ├── ABF10.pt
    │   ├── ABF11.pt
    │   └── …   
    ├── train/                # extracted from HO3D_v3.zip
    └── train.txt             # extracted from HO3D_v3.zip
```