...
class GenericServer(torch.nn.Module):
    ...
    def forward(self, scene_scale, transl, thetas, betas, absolute=False, can2world_mat=None):
        out = {}

        # ignore betas if v_template is provided
        if self.v_template is not None:
            betas = torch.zeros_like(betas)
        # outputs in the canonical space
        ...
        out["verts"] = outputs.vertices.clone() # verts in hand deform space
        out["jnts"] = outputs.joints.clone() # joints in hand deform space
        tf_mats = outputs.T.clone()       # transformation from canonical space to deform space
        if can2world_mat is not None:
            out["verts"] = transform_points(out["verts"], can2world_mat) # verts in world space
            out["jnts"] = transform_points(out["jnts"], can2world_mat) # joints in world space
            tf_mats = can2world_mat[:,None,:,:] @ tf_mats # transformation from canonical space to world space
        else:
