...
class NeuSVolumeRenderer(VolumeRenderer):
    ...
    def forward(
        ...
        **kwargs
        ...
        # # deformed mesh of output is in the world space
        # output = self.hand.server(
        #     kwargs["hand_scale"],
        #     kwargs["hand_transl"],
        #     kwargs["hand_full_pose"],
        #     kwargs["hand_betas"],
        #     can2world_mat=kwargs["hand_h2o"],
        # )
        
        # debug_img_f = debug.debug_world2pix(self.cfg, output, int(kwargs["image_index"].cpu()), "right")
        # debug_mesh_fs = debug.debug_deformer_mano(self.cfg, output, int(kwargs["image_index"].cpu()), self.hand.server, self.hand.deformer, "right")        
        ...
        # # positions in the camera space, and positions_c in the hand canonical space
        # positions, outlier_mask = self.hand.deformer.forward(
        #     positions[None,...], output["tfs"], return_weights=False, inverse=True, verts=output["verts"]
        # )
