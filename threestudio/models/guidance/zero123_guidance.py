...
class Zero123Guidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        do_3d_guidance_only: bool = False
    ...
    def configure(self) -> None:
        ...
        threestudio.info(f"Condition image path: {self.cfg.cond_image_path}")
    ...
    def __call__(
        ...
        guidance_eval=False,
        visibility_binarys=None,
        visibility_colors=None,
        **kwargs,
    ):
        ...
        # print(f"min_step {self.min_step}, max_step {self.max_step}, t {t}")
        ...
        if not self.cfg.do_3d_guidance_only and len(visibility_binarys) > 0 and len(visibility_colors) > 0:
            visibility = (visibility_colors[:,:,:,:,0] > 0.5).float()
            visibility_sum = visibility.sum(axis=(1,2,3))
            visibility_mask_sum = visibility_binarys.sum(axis=(1,2,3))
            visibility_ratio = visibility_sum / visibility_mask_sum
            # novel_view_w = torch.where(visibility_ratio > 0.3, 0.1, 1.0)
            # novel_view_w = (1 + torch.cos(torch.pi * visibility_ratio)) / 2
            variance = 0.3
            novel_view_w = torch.exp(-((visibility_ratio)**2) / (2 * variance))
            novel_view_w = torch.clip(novel_view_w, 0.0, 1.0)
            # for x in zip(visibility_ratio, novel_view_w, azimuth):
            #     print(f"visibility_ratio {x[0]:.2f} novel_view_w {x[1]:.2f}, azimuth {x[2]:.2f}")
            novel_view_w = novel_view_w[:,None,None,None]
            # visibility = F.interpolate(visibility, (32, 32), mode="bilinear", align_corners=False)
            # w_visibility = visibility * 0.2 + (1.0 - visibility) * 1.0
                
            mse_loss_elementwise = F.mse_loss(latents, target, reduction="none")
            weighted_mse = mse_loss_elementwise * novel_view_w
            loss_sds = 0.5 * weighted_mse.sum() / batch_size
        else:
